...
final class AudioFileStreamProcessor {
    ...
    private var flacSeekTable = FlacSeekTable()
    private var seekFramesToDiscard: Int64 = 0
    ...
    func openFileStream(with fileHint: AudioFileTypeID) -> OSStatus {
        flacSeekTable = FlacSeekTable()
        seekFramesToDiscard = 0
    ...
    func parseFileStreamBytes(data: Data) -> OSStatus {
        ...
        if !flacSeekTable.complete {
            flacSeekTable.consume(data)
            if let duration = flacSeekTable.duration, let entry = playerContext.audioReadingEntry {
                entry.lock.withLock { entry.exactDuration = duration }
            }
        }
    ...
    func processSeek() {
        ...
        seekFramesToDiscard = 0
        if let plan = flacSeekTable.plan(time: readingEntry.seekRequest.time,
                                         outputSampleRate: outputAudioFormat.mSampleRate,
                                         fileLength: readingEntry.length) {
            seekByteOffset = plan.byteOffset
            seekFramesToDiscard = plan.framesToDiscard
            readingEntry.lock.withLock { readingEntry.seekTime = plan.time }
            discontinuous = true
        } else {
    ...
    func propertyPacketsProc(
        ...
        // 从最近定位点到目标之间的 PCM 只解码不播放，也不计入播放进度。
        if seekFramesToDiscard > 0 {
            let capacity = 4096
            let bytesPerFrame = Int(outputAudioFormat.mBytesPerFrame)
            let scratch = UnsafeMutableRawPointer.allocate(byteCount: capacity * bytesPerFrame,
                                                           alignment: MemoryLayout<Float>.alignment)
            defer { scratch.deallocate() }
            while seekFramesToDiscard > 0 {
                if entry.seekRequest.requested || playerContext.internalState == .disposed ||
                    playerContext.internalState == .pendingNext || playerContext.internalState == .stopped {
                    return
                }
                var frames = UInt32(min(Int64(capacity), seekFramesToDiscard))
                var buffers = AudioBufferList(mNumberBuffers: 1, mBuffers: AudioBuffer(
                    mNumberChannels: outputAudioFormat.mChannelsPerFrame,
                    mDataByteSize: frames * outputAudioFormat.mBytesPerFrame, mData: scratch))
                let result = AudioConverterFillComplexBuffer(converter, _converterCallback,
                    &convertInfo, &frames, &buffers, nil)
                seekFramesToDiscard -= Int64(frames)
                if result == AudioConvertStatus.done.rawValue { return }
